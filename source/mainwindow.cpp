...
void MainWindow::createActions() //меню + кнопки
{
    ...
    for (auto & recentFileAct : recentFileActs) { //aaaaaaaaaaaaaaaaaaaa
    ...
    auto *cutAct = new QAction(cutIcon, tr("&Cut"), this);
    ...
    auto authorAct = new QAction(tr("&AIRA-D"), this);
    authorAct->setStatusTip(tr("=]"));
    editMenu->addAction(authorAct);
...
void MainWindow::loadFile(const QString &fileName)
{
    ...
    if (!file.open(QFile::ReadOnly | QFile::Text)) {
        QMessageBox::warning(this, tr("Manager"), tr("Cannot read file %1:\n%2.").arg(QDir::toNativeSeparators(fileName), file.errorString()));
        return;
    }
...
// я енотик полоскун, полоскаю свой 3.14...
    QGuiApplication::setOverrideCursor(Qt::WaitCursor);
